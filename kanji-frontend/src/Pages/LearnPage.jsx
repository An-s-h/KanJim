...
export default function LearnPage() {
  ...
  const [selectedKanji, setSelectedKanji] = useState(null); // modal state
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      ...
      {/* Header Section */}
      <div className="relative px-6 pt-[120px]  pb-12 text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
          {kanjis.map((k, index) => (
            ...
            >
              ...
              <div className="text-center mb-4">
                <h3 className="text-lg font-semibold text-purple-300 capitalize">{k.meaning}</h3>
              </div>

              <div className="space-y-3">
                {k.onyomi?.length > 0 && (
                  <div className="bg-white/5 rounded-lg p-3 border border-white/10">
                    <div className="text-xs font-semibold text-blue-400 mb-1">音読み (On'yomi)</div>
                    <div className="text-sm text-gray-300 font-medium">{k.onyomi.join(" • ")}</div>
                  </div>
                )}
                {k.kunyomi?.length > 0 && (
                  <div className="bg-white/5 rounded-lg p-3 border border-white/10">
                    <div className="text-xs font-semibold text-green-400 mb-1">訓読み (Kun'yomi)</div>
                    <div className="text-sm text-gray-300 font-medium">{k.kunyomi.join(" • ")}</div>
                  </div>
                )}
              </div>
